document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderStudy() {
    ...
    const photoHTML = card.photo
      ? `<img src="${card.photo}" style="width:100%;max-height:200px;border-radius:10px;object-fit:cover;margin-bottom:14px" />`
      : "";
    ...
    if (card.type === "multiple") {
      ...
      const choicesHTML = card.choices.map((choice, idx) =>
        `<div class="choice-option" data-choice-index="${idx}">${safeText(choice)}</div>`
      ).join("");
      studyTextFront.innerHTML =
        `${photoHTML}<div>${safeText(card.q)}</div>` +
        `<div style="margin-top:14px;display:flex;flex-direction:column;gap:10px">${choicesHTML}</div>`;

      const correctIdx = parseInt(card.a, 10);
      studyBadgeBack.textContent = "ANSWER";
      studyTextBack.innerHTML =
        `${photoHTML}<div style="color:var(--primary);font-weight:950">Correct Answer:</div>` +
        `<div style="margin-top:10px">${safeText(card.choices[correctIdx])}</div>`;

      const choiceEls = studyCard.querySelectorAll(".choice-option");
      choiceEls.forEach(el => {
        el.addEventListener("click", (ev) => {
          ...
          if (isCorrect) {
            el.style.backgroundColor = "#16a34a33";
            el.style.borderColor = "#16a34a";
            el.style.color = "#15803d";
          } else {
            el.style.backgroundColor = "#dc262633";
  ...
  function nextStudy(fromTimeout = false) {
    ...
    if (timed.active && !fromTimeout) {
      timed.reviewed += 1;
      if (timed.remaining > 0) timed.answeredInTime += 1;
    }
    if (!fromTimeout) study.answeredCount += 1;

    study.index += 1;
    study.side = "q";
    renderStudy();
    if (timed.active && study.index < deck.cards.length) startCardTimer();
  }
  ...
  function openManage(deckId) {
    ...
    manageList.innerHTML = deck.cards.map((c, idx) => {
      const photoHTML = c.photo
        ? `<img src="${c.photo}" style="width:100%;max-height:100px;border-radius:8px;object-fit:cover;margin-top:10px" />`
        : "";
      if (c.type === "multiple") {
        return `
